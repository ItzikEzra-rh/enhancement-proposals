...
- The fulfillment API endpoints listed in Section 3.1 (FR-45) are already implemented and stable—no API changes are required for this UI work
...
## 6. Dependencies
- fulfillment-service REST gateway endpoints must be available and stable (GET/POST/PATCH/DELETE for /v1/virtualnetworks, /v1/subnets, /v1/securitygroups, /v1/publicips)
- protobuf-generated TypeScript types must be available in `libs/types/src/osac/public/v1/` for VirtualNetwork, Subnet, SecurityGroup, PublicIP, PublicIPPool
...
- **Mitigation:** Confirm the current PatternFly version used by OSAC UI before implementation. If OSAC UI is on PatternFly 5, implement components using PF5 and plan a migration path to PF6 in a future phase. If OSAC UI is already on PF6, proceed as specified.
