...
7. **Step 4 — Access:** Per-kind access step component with `ssh_public_key`/`ssh_key` and `pull_secret` (clusters) as `StringFieldDefinition` fields. Both default to editable.
   For VM catalog items, the UI automatically includes the field definition in the API payload without showing it in a wizard step. The Catalog Item payload represents it as `fields.network_attachments` using a typed `ComputeNetworkAttachmentListFieldPolicy`; an editable policy may include `default_value: { items: [...] }`, while `default_value` may be omitted. During provisioning, the tenant sends `catalog_item` together with tenant-supplied `network_attachments`; fulfillment resolves the policy, applies default-network injection when the field is omitted as defined by OSAC-1435, and performs final resource validation. The absence of a JSON `validation_schema` does not disable typed policy checks or resource validation. Bare Metal catalog items have no networking fields.
...
`ClusterNetworkingStep` with `pod_cidr` and `service_cidr` as `StringFieldDefinition` fields. This step is not shown for VM or Bare Metal catalog items. For VM catalog items, the UI omits a wizard control but emits `fields.network_attachments` as the typed `ComputeNetworkAttachmentListFieldPolicy`. Its `default_value` may be omitted or may contain typed `items`; it is not a statement that the field is unvalidated. During provisioning, the tenant sends `catalog_item` and any tenant-supplied `network_attachments`; fulfillment resolves the policy, applies default-network injection when needed, and performs final resource validation. Bare Metal catalog items have no networking fields.
...
**Network attachments handling (VM only):** The `network_attachments` field is not shown in any wizard step. The Catalog Item create payload includes it as `fields.network_attachments`, a typed `ComputeNetworkAttachmentListFieldPolicy`; an editable policy can carry `default_value: { items: [...] }`, but `default_value` can also be omitted. The provisioning request carries `catalog_item` and tenant-supplied `network_attachments`. Fulfillment resolves the policy, applies default-network injection when needed, and performs final resource validation. Omitting a JSON `validation_schema` does not bypass typed policy checks or resource validation; existing workload attachments are not updated in place.
...
- Network attachments auto-inclusion (VM only): verify the VM wizard omits the field, the Catalog Item payload uses typed `fields.network_attachments` with an optional `default_value.items`, and the provisioning payload carries `catalog_item` plus tenant-supplied `network_attachments`; verify omitted JSON `validation_schema` does not bypass typed policy or resource validation; verify Bare Metal has no networking fields; verify Cluster uses pod_cidr/service_cidr in Networking step
