...
7. **Cloud-native and metering air-gap compatible** — all metering components run as containerized workloads on OpenShift with no external SaaS dependencies [PRD: CAP-14]. This packaging property does not change the [Unified Networking deployment support boundary](/enhancements/OSAC-1433-unified-networking/design.md#deployment-support-boundary), which requires connected OSAC networking deployments.
...
**Metering Air-Gap Compatibility:** All metering components run on-premise within the OSAC installation. No outbound calls to external CDNs, license servers, or SaaS telemetry endpoints. The billing provider endpoint may be on-premise or an on-premise proxy. This does not make an OSAC networking deployment air-gap compatible; networking remains governed by the [Unified Networking deployment support boundary](/enhancements/OSAC-1433-unified-networking/design.md#deployment-support-boundary).
