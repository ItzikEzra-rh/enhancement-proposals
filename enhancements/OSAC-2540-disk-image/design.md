...
- `GuestOSFamily` replaces the existing `is_windows` boolean on `ComputeInstanceSpec` — not a new concept, just a better API shape. An enum is more extensible than a boolean (adding a third OS family doesn't require a new field), and `GUEST_OS_FAMILY_LINUX` is self-documenting where `is_windows: false` isn't. It uses the standard OSAC enum naming convention with `_UNSPECIFIED = 0`. **Data flow:** DiskImage stores the enum value → the ComputeInstance reconciler resolves the DiskImage and maps the enum to the CRD's `GuestOSFamily` string field (`"windows"` or `"linux"`) → the osac-operator passes the full CR to AAP via extra_vars → the `ocp_virt_vm` Ansible role branches on `guest_os_family == 'windows'` to configure the KubeVirt VirtualMachine domain (Windows: UEFI + Secure Boot, TPM, Hyper-V clock timers, sysprep initialization; Linux: BIOS defaults, cloud-init). The CRD field and everything downstream is unchanged by DiskImage — only the input source moves from `ComputeInstanceSpec.is_windows` to `DiskImageSpec.guest_os_family`. See [KubeVirt domain spec](https://kubevirt.io/api-reference/main/definitions.html#_v1_domainspec) and [Windows VM guide](https://kubevirt.io/user-guide/virtual_machines/windows_virtio_drivers/).
  **GuestOSFamily VM configuration summary:** The `guest_os_family` value determines KubeVirt VirtualMachine domain configuration via the `ocp_virt_vm` AAP role ([`create_build_spec.yaml`](https://github.com/osac-project/osac-aap/blob/main/collections/ansible_collections/osac/templates/roles/ocp_virt_vm/tasks/create_build_spec.yaml)). User-visible differences:

  | Aspect | LINUX (default) | WINDOWS |
  |--------|----------------|---------|
  | Firmware | BIOS (KubeVirt default) | UEFI with Secure Boot |
  | TPM | No | Yes |
  | RNG device | No | Yes |
  | Clock | KubeVirt defaults | UTC + Hyper-V timers (hpet, pit, rtc) |
  | Initialization | cloud-init (`cloudInitNoCloud`) | Sysprep (`unattend.xml` via SATA CD-ROM) |
  | SSH access | SSH public key via `accessCredentials` | Not supported |
  | Remote access | Port 22/tcp (SSH) | Port 3389/tcp (RDP) |
  | Default boot disk | 10 GiB | 40 GiB |
  | Default image | `quay.io/containerdisks/fedora:latest` | None — must be explicitly provided |
  | Ready timeout | 600s | 900s |

  These behaviors are implemented in the `ocp_virt_vm` Ansible role and are not changed by DiskImage — they pre-exist. API reference documentation must describe these user-visible effects for each `GuestOSFamily` value so users understand what selecting LINUX vs WINDOWS means for their VM.
